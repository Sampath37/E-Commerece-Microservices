...
public class InventoryService {
	...
	public Inventory addStock(Inventory inventory) {
		...
		return inventoryRepository.findByProductCode(inventory.getProductCode()).map(existing -> {
	...
	@CachePut(value = "inventoryCache", key = "#productCode")
	@CacheEvict(value = "inventoryListCache", allEntries = true)
	public Inventory updateStock(String productCode, Integer newStock) {
		return inventoryRepository.findByProductCode(productCode).map(existing -> {
			...
			if (productCode != null && quantity != null && orderId != null) {
				inventoryRepository.findByProductCodeAndStatus(productCode, "ACTIVE").ifPresentOrElse(inventory -> {
					if (inventory.getStock() >= quantity) {
						...
						InventoryEventDto failedEvent = new InventoryEventDto(orderId, productCode, "FAILED",
								"INSUFFICIENT_STOCK");
